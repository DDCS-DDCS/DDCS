...
parser.add_argument('--up_gradual', type=int, default=20,  
                    help='how many epochs for linear up rate, can be 5, 10, 15. This parameter is equal to Tk for R(T) in Co-teaching paper.')
...
def main():
    ...
    if args.noise == 'idn':
        ...
        dataset = build_cifar100n_dataset(os.path.join('./data/'), CLDataTransform(transform['cifar_train'], transform['cifar_train_strong_aug']),
    ...
    if not os.path.exists(r'./DDCS'):
        os.makedirs(r'./DDCS')

    fp = 'DDCS/{}-old-{}-{}-{}.csv'.format(args.noise, args.dataset, args.noise_type, args.noise_rate)
    ...
    with open('result/{}-old-{}-{}-{}-avg.csv'.format(args.noise, args.dataset, args.noise_type, args.noise_rate), 'a',
              newline='') as f:
